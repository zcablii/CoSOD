...
def get_test_info(sal_mode='e'):
    if sal_mode == 'e':
        ...
        image_root = '../../CoSOD/Data/DUTS_class/img/'
        image_source = ''
...
def main(config):
    if config.mode == 'train':
        ...
        img_dir_paths = list(map(lambda x: os.path.join(test_root, x), classes))
        for root in img_dir_paths:
            config.test_root = root
            config.test_list = os.listdir(root)
            test_loader = get_loader(config, mode='test')
            config.test_fold = os.path.join('res/',root[17:]+'/')
            if not os.path.exists(config.test_fold): 
                os.mkdir(os.path.join('res/',root[17:]+'/'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_fold', type=str, default='test_res') # Test results saving folder
